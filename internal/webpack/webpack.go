...
import (
	"errors"
	"go-web-base/internal/webpack/helper"
	"go-web-base/internal/webpack/reader"
	"html/template"
	"log"
	"strings"
)
...
// DevHost assets-dev-server host:port
var DevHost = "localhost:3808"

// FsPath filesystem path to public assets dir
var FsPath = "./dist"

// WebPath http path to public assets dir
var WebPath = "dist"

// Plugin assets plugin to use, can be stats or manifest
var Plugin = "manifest"
...
// Config is an instance of go-webpack configuration to use with multiple manifest files (multiple assets configs)
type Config struct {
	// DevHost assets-dev-server host:port
	DevHost string
	// FsPath filesystem path to public assets dir
	...
	Verbose bool
	// IsDev - true to use assets-serve or assets-dev-server, false to use filesystem and manifest.json
	IsDev bool
}

// AssetHelper renders asset tag with url from assets manifest to the page. This is a default assethelper, exported at package level. You can also get your own AssetHelper with assets.GetAssetHelper
var AssetHelper func(string, bool) (template.HTML, error)
...
func ErrorFunction(err error) func(string, bool) (template.HTML, error) {
	log.Println("go-webpack: error:", err)
	return func(string, bool) (template.HTML, error) {
		return "", err
	}
}

// GetAssetHelper returns an asset helper function based on your config, for use with multiple assets manifests
func GetAssetHelper(conf *Config) func(string, bool) (template.HTML, error) {
	preloadedAssets := AssetList{}

	var err error
	if conf.IsDev {
		// Try to preload manifest, so we can show an error if assets-dev-server is not running
...
func createAssetHelper(conf *Config, preloadedAssets AssetList) func(string, bool) (template.HTML, error) {
	return func(key string, shouldDefer bool) (template.HTML, error) {
		var err error
		...
		for _, s := range v {
			if strings.HasSuffix(s, "."+kind) {
				buf = append(buf, helper.AssetTag(kind, s, shouldDefer))
			} else {
